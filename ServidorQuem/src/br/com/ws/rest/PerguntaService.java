...
public class PerguntaService {
	...
	@GET
	@Path("/miniJogo/{idMiniJogo}")
	@Produces(MediaType.APPLICATION_JSON)
	public List<Pergunta> listarPerguntasPorIdMiniJogo(@PathParam("idMiniJogo")int idMiniJogo){
		int exceptionNumber = 500;
		try{
			List<Pergunta> perguntas = perguntaDAO.listarPerguntasPorIdMiniJogo(idMiniJogo);
			if (perguntas.isEmpty()) {
					exceptionNumber = 404;
					throw new Exception("No perguntas registred");
				}
